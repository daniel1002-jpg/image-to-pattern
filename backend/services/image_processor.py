...
"""
Process image bytes and return palette and grid pattern.
"""
def process_image_to_pattern(image_bytes: bytes, width: int, n_colors: int) -> dict:
    ...
    return {
        ...
        "palette": [rgb_to_hex(color) for color in kmeans.cluster_centers_],
        "grid": kmeans.labels_.reshape(new_height, width).tolist()
    }
